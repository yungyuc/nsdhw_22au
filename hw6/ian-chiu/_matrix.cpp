 ...
 public:
    Matrix(size_t m, size_t n) : nrow(m), ncol(n), m_buffer(m * n) {
        for (size_t i = 0; i < m * n; i++) {
            m_buffer.mutable_at(i) = 0.0;
        }
        m_buffer.resize({nrow, ncol});
    }

    double operator()(size_t row, size_t col) const {
        return m_buffer.at(row, col);
    }
    double &operator()(size_t row, size_t col) {
        return m_buffer.mutable_at(row, col);
    }
    double operator()(size_t index) const { return m_buffer.at(index); }
    double &operator()(size_t index) { return m_buffer.mutable_at(index); }
...
Matrix multiply_mkl(const Matrix &mat1, const Matrix &mat2) {
    ...
    cblas_dgemm(CblasRowMajor /* const CBLAS_LAYOUT Layout */
                ...
                ,
                mat1.m_buffer.data(0) /* const double *a */
                ,
                mat1.ncol /* const MKL_INT lda */
                ,
                mat2.m_buffer.data(0) /* const double *b */
                ,
                mat2.ncol /* const MKL_INT ldb */
                ,
                0.0 /* const double beta */
                ,
                ret.m_buffer.mutable_data(0) /* double * c */
                ,
...
PYBIND11_MODULE(_matrix, m) {
    py::class_<Matrix>(m, "Matrix")
        ...
        .def_readwrite("array", &Matrix::m_buffer);
