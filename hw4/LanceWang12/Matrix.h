...
public:
    ...
    template<class U>
    MyAllocator(const MyAllocator<U>& rhs) noexcept : my_counter(rhs.my_counter) {}
    ...
    constexpr bool operator==(const MyAllocator<T>& rhs) const{
        return (this->my_counter == rhs.my_counter);
    }

    friend ostream& operator<<(ostream& out, const MyAllocator<T>& rhs){
        out << "MyMyAllocator: bytes = " << rhs.bytes() << '\n'\
